...
def plot_heatmap():
    ...
    fig = plt.figure()
    ax = fig.add_subplot()
    # fig, ((ax, ax1), (ax2, nax)) = plt.subplots(
    #     nrows=2, 
    #     ncols=2, 
    #     gridspec_kw={"height_ratios":[1, 0.1], "width_ratios":[1, 0.1],},
    #     sharey='row', 
    #     sharex='col'
    # )
    ...
    # x = np.linspace(0,len(yticks))
    # y = np.array(result.iloc[-1 ,:-1].tolist())

    # ax2.imshow(y[np.newaxis,:], cmap="plasma", aspect="auto")
    # # ax2.set_xticks(_y, yticks)

    
    # x = np.array(result.iloc[:-1,-1].tolist())
    # y = np.linspace(0,len(xticks))

    # ax1.imshow(x[np.newaxis,:], cmap="plasma", aspect="auto")
